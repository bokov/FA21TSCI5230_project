...
packageList <- c("knitr","Publish","ggplot2","rio","GGally","dplyr","pander","readr","synthpop", "rmarkdown", "readxl", "tibble", "broom", "scales", "ModelMetrics","printr");
...
fitRMSE <- augment(fit) %>% with(ModelMetrics::rmse(.fitted,NOREADMISSION))
#table(is.na(predict(fit)))

```

The root mean square error is `r fitRMSE`. Starting model results:
...
plot(dtest$NOREADMISSION ~ predict(fitBIC, dtest))

fitBICRMSE <- augment(fitBIC) %>% with(ModelMetrics::rmse(.fitted,NOREADMISSION))
#table(is.na(predict(fitBIC)))

```

The RMSE for the model with stepwise-selected variables was `r fitBICRMSE`. The 
BIC model results were as follows:
